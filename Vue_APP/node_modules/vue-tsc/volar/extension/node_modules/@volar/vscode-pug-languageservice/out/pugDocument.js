...
function parsePugDocument(pugTextDoc, htmlLs) {
    ...
    function addClassesOrStyles(attrs, attrName) {
        ...
        if (escapeAttrs.length) {
            codeGen.addCode(attrName, getDocRange(escapeAttrs[0].line, escapeAttrs[1].column, attrName.length), SourceMap.Mode.Offset, undefined, escapeAttrs.slice(1).map(attr => getDocRange(attr.line, attr.column, attrName.length)));
        }
        else {
