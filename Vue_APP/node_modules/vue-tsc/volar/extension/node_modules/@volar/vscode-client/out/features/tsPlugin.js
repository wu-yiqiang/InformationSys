...
async function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('volar.action.switchTsPlugin', async () => {
        ...
        const select = await formatAll_1.userPick(options);
        if (select === undefined)
            return; // cancle
        if (select === 0) {
    ...
    function onConfigUpdated() {
        ...
        updateStatusBar();
        if (vscode.workspace.getConfiguration('volar').get('tsPluginStatus')) {
            statusBar.show();
        }
        else {
            statusBar.hide();
    ...
    async function showReload(enabled) {
    ...
    function getTsPluginStatus() {
        return vscode.workspace.getConfiguration('volar').get('tsPlugin');
    }
}
